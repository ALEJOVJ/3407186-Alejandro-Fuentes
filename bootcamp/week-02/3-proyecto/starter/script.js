// ============================================
// PROYECTO SEMANA 02 - TIENDA DE ROPA INFANTIL
// ============================================
...
const CATEGORIES = {
  ...
  boy: { name: 'Niño', emoji: '🧒' },
  girl: { name: 'Niña', emoji: '👧' },
  shoes: { name: 'Calzado', emoji: '👟' },
  accessories: { name: 'Accesorios', emoji: '🎒' },
};
...
// ============================================
  ...
  const loadItems = () => {
  return JSON.parse(localStorage.getItem('kidsClothes') ?? '[]');
};

  const saveItems = itemsToSave => {
  localStorage.setItem('kidsClothes', JSON.stringify(itemsToSave));
};
...
// ============================================
  ...
  const newItem = {
    ...
    category: itemData.category ?? 'baby',
...
const formatDate = dateString => {
 ...
 const renderItem = item => {
  const { id, name, description, category, priority, active, createdAt, size, price } = item;
 ...
 const renderStats = stats => {
  ...
  const categoryStats = Object.entries(stats.byCategory)
    .map(([cat, count]) => `${getCategoryEmoji(cat)} ${CATEGORIES[cat].name}: ${count}`)
    .join(' | ');
...
// ============================================
 ...
 const handleFormSubmit = e => {
  ...
  const itemData = { name, description, category, priority, size, price };
...
// ============================================
 ...
 const init = () => {
  ...
  console.log('✅ Tienda de ropa infantil inicializada');
};
